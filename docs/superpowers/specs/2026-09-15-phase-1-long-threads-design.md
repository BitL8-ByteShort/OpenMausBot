...
1. **Context size per task.** The store keeps `usage.lastInput`: the input tokens of the last settled turn, as the engine reported them. That number is the context the next turn will carry. Engines that report no usage (some ACP agents, pi on some hosts) get an estimate: bytes of the thread's replayable text since the last compaction ÷ 4.
