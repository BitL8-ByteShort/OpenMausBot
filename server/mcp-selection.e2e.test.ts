...
it("imports atomically and mounts the selected MCP servers in fresh direct and room sessions", async () => {
  await withFixture("happy", async ({ api, control, dump, clearDump }) => {
    ...
    // A well-formed https address is a VALID remote server now, so the entry
    // that must be refused has to be genuinely unparseable — otherwise this
    // stops testing that one bad entry rejects the whole batch.
    expect((await api("POST", "/api/mcp/servers/import", { json: JSON.stringify({ newone: command, bad: { url: "not a url" } }) })).status).toBe(400);
