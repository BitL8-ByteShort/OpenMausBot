...
async function startTurn(
  ...
  void (async () => {
    try {
      ...
      if (agentsMounted) {
        integrations.agents = agentsIntegration(bot.id, threadId, commsDepth, skillAuthoring, dispatchClaimId, opts?.coordination?.id, boundedCoordination, boardEnabled(cfg));
      }
